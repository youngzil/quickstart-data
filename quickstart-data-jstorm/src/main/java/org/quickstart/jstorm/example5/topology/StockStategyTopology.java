...
public class StockStategyTopology {
    public static void main(String[] args) throws Exception {
        ...
        builder.setBolt("stock-stategy-1", new StockStrategyBolt1(), 2).setNumTasks(2).shuffleGrouping("stock-filter");
        // 大单买(stock-stategy-2): 选出股票的买5档总手数大于卖5档口总手数100倍时的股票
        builder.setBolt("stock-stategy-2", new StockStrategyBolt2(), 2).setNumTasks(2).shuffleGrouping("stock-filter");
        // 放巨量(stock-stategy-3): 选出在10秒内成交量超过1000万时的股票
        builder.setBolt("stock-stategy-3", new StockStrategyBolt3(), 2).setNumTasks(2).shuffleGrouping("stock-filter");
        // 统计报表
        builder.setBolt("report", new ReportBolt(), 1).setNumTasks(2).shuffleGrouping("stock-stategy-1").shuffleGrouping("stock-stategy-2").shuffleGrouping("stock-stategy-3");
