...
public class ElasticSearchToolsTest {
  ...
  static {
    // 设置集群名字
    Settings settings = Settings.settingsBuilder().put("cluster.name", "search")
        .put("client.transport.sniff", true).build();
    try {
      // 初始化连接客户端
      client = new TransportClient.Builder().settings(settings).build().addTransportAddress(
          new InetSocketTransportAddress(new InetSocketAddress("192.168.201.5", 9300)))
          .addTransportAddress(
              new InetSocketTransportAddress(new InetSocketAddress("192.168.201.6", 9300)))
          .addTransportAddress(
              new InetSocketTransportAddress(new InetSocketAddress("192.168.201.7", 9300)));
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public static void filterQueryString() {
    ...
    BoolQueryBuilder boolQuery = QueryBuilders.boolQuery()
        ...
        .filter(QueryBuilders
            .queryStringQuery("+mtitle:北京奇虎科技     +dtime:[1443196800000 TO 1451059200000] ")
  ...
  public static void countDistinctByField() {
    ...
    search.setQuery(QueryBuilders.queryStringQuery(
        "@timestamp:[ " + new DateTime(2016, 8, 8, 0, 0, 0).getMillis() + " TO " + new DateTime(
            2016, 8, 15, 0, 0, 0).getMillis() + "}"));
    search.setSize(0);
    // 一级分组字段
    ...
    dateagg.timeZone(
        "Asia/Shanghai");// 设置时区，注意如果程序部署在其他国家使用时，使用Joda-Time来动态获取时区 new DateTime().getZone()
  ...
  public static void testQuery() throws Exception {
    ...
    search.setQuery(QueryBuilders
        .queryStringQuery("+data.subjectName:* -data.subjectName:" + subjectName + "  "));
    showResult(search);
  }
  ...
  public static void testTwoAggString() throws Exception {
    ...
    SearchRequestBuilder search = client.prepareSearch("userlog*").setTypes("logs");
    search.setQuery(QueryBuilders.queryStringQuery(
        "@timestamp:[ " + new DateTime(2016, 8, 10, 0, 0, 0).getMillis() + " TO " + new DateTime(
            2016, 8, 11, 0, 0, 0).getMillis() + "}"));
    // 一级分组字段
    DateHistogramBuilder dateagg = AggregationBuilders.dateHistogram("dateagg");
    dateagg.field("@timestamp");// 聚合时间字段
    dateagg.interval(DateHistogramInterval.HOUR);// 按小时聚合
