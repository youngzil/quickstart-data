...
import org.quickstart.elasticsearch.jest.example2.CsdnBlog;
...
public class JestExample {
    ...
    public void getClient() throws Exception {
        ...
        factory.setHttpClientConfig(new HttpClientConfig.Builder(urlList)
                ...
                .gson(new GsonBuilder().setDateFormat("yyyy-MM-dd'T'hh:mm:ss").create()).connTimeout(1500).readTimeout(3000).multiThreaded(true).build());
    ...
    public void createIndexMapping() throws Exception {
        ...
        String source = "{\"" + typeName + "\":{\"properties\":{" + "\"author\":{\"type\":\"string\",\"index\":\"not_analyzed\"}" + ",\"title\":{\"type\":\"string\"}"
                + ",\"content\":{\"type\":\"string\"}" + ",\"price\":{\"type\":\"string\"}" + ",\"view\":{\"type\":\"string\"}" + ",\"tag\":{\"type\":\"string\"}"
    ...
    public void bulkIndex() throws IOException {
        ...
        Bulk bulk = new Bulk.Builder().defaultIndex(indexName).defaultType(typeName)
                .addAction(Arrays.asList(new Index.Builder(csdnBlog1).build(), new Index.Builder(csdnBlog2).build(), new Index.Builder(csdnBlog3).build(), new Index.Builder(csdnBlog4).build()))
                .build();
    ...
    public void updateDocument() throws Exception {
        ...
        String script = "{" + "    \"doc\" : {" + "        \"title\" : \"" + article.getTitile() + "\"," + "        \"content\" : \"" + article.getContent() + "\"," + "        \"author\" : \""
                + article.getAuthor() + "\"," + "        \"view\" : \"" + article.getView() + "\"," + "        \"tag\" : \"" + article.getTag() + "\"," + "        \"date\" : \""
    ...
    public void suggest() throws Exception {
        ...
        Suggest suggest =
                new Suggest.Builder("{" + "  \"" + suggestionName + "\" : {" + "    \"text\" : \"the amsterdma meetpu\"," + "    \"term\" : {" + "      \"field\" : \"body\"" + "    }" + "  }" + "}")
                        .build();
