...
public class IndexAPI extends ElasticsearchRestHighClient {
    ...
    public void testIndex() throws Exception {
        ...
        builder.endObject();
        IndexRequest indexRequest2 = new IndexRequest("posts", "doc", "1").source(builder); // XContentBuilder 对象作为文档源，由 Elasticsearch 内置的帮助器生成 JSON 内容

        IndexRequest indexRequest3 = new IndexRequest("posts", "doc", "1").source("user", "kimchy", "postDate", new Date(), "message", "trying out Elasticsearch"); // 以键值对对象作为文档来源，它自动转换为 JSON 格式
        ...
        IndexRequest request5 = new IndexRequest("posts", "doc", "1").source("field", "value").version(1);
        try {
        ...
        IndexRequest request6 = new IndexRequest("posts", "doc", "1").source("field", "value").opType(DocWriteRequest.OpType.CREATE);
        try {
            IndexResponse response = client.index(request);
        } catch (ElasticsearchException e) {
            if (e.status() == RestStatus.CONFLICT) {
