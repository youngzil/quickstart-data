...
public class ElasticsearchRestClient {
    ...
    public void setUp() throws Exception {
        ...
        client = RestClient.builder(//
                ...
                .setMaxRetryTimeoutMillis(10000)// 4 也调整最大重试超时时间（默认为30秒）。设置在同一请求进行多次尝试时应该遵守的超时时间。默认值为30秒，与默认`socket`超时相同。 如果自定义设置了`socket`超时，则应该相应地调整最大重试超时。
                ...
                .setHttpClientConfigCallback(new RestClientBuilder.HttpClientConfigCallback() {
                    ...
                    public HttpAsyncClientBuilder customizeHttpClient(HttpAsyncClientBuilder httpClientBuilder) {
                        return httpClientBuilder// Apache Http Async Client 默认启动一个调度线程，连接管理器使用多个worker线程,线程的数量和CPU核数量相同（等于 Runtime.getRuntime().availableProcessors()
                                ...
                                .setProxy(new HttpHost("proxy", 9000, "http")); // 设置修改 http 客户端配置的回调（例如：ssl 加密通讯，或其他任何
